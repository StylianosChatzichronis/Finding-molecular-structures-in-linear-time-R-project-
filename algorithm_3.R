...
const=1;
while((left<=right)&&(abs(F1-F2$value[m])>=const)){
  ...
  if(abs(F1-F2$value[right])<abs(F1-F2$value[m])){
    ...
    if( const<1){
      ...
      r =rmsd(coordinates_Q$xyz,coordinates_P$xyz[(sum(coordinates_P$size[1:(F2$pos[left])])):(length(coordinates_Q$xyz)+(sum(coordinates_P$size[1:(F2$pos[left])])) -1)]);
      if(r<1){
    ...
    if(abs(F1-F2$value[left])<abs(F1-F2$value[m])){
      ...
      if( const<1){  print(const);
        const=abs(F1-F2$value[m]);
        r =rmsd(coordinates_Q$xyz,coordinates_P$xyz[(sum(coordinates_P$size[1:(F2$pos[right])])):(length(coordinates_Q$xyz)+(sum(coordinates_P$size[1:(F2$pos[right])])) -1)]);
        if(r<1){
          #rmsd_var = c(rmsd_var$value,r);
          rmsd_var$value = c(rmsd_var$value,r);
          rmsd_var$name = c(rmsd_var$name,coordinates_P$name[m]);
      ...
      right = m- 1;
        ...
        if( const<1){
          const=abs(F1-F2$value[m]);
          r =rmsd(coordinates_Q$xyz,coordinates_P$xyz[(sum(coordinates_P$size[1:(F2$pos[m])])):(length(coordinates_Q$xyz)+(sum(coordinates_P$size[1:(F2$pos[m])])) -1)]);
          if(r<1){
            #rmsd_var = c(rmsd_var$value,r);
            rmsd_var$value = c(rmsd_var$value,r);
            rmsd_var$name = c(rmsd_var$name,coordinates_P$name[m]);
...
while(u>=left){
  ...
  r =rmsd(coordinates_Q$xyz,coordinates_P$xyz[(F2$pos_in_protein[u]):(length(coordinates_Q$xyz)+F2$pos_in_protein[u] -1)]);
  ...
  if(r<1){
    print(r)
    print(F2$pos_in_protein[u])
    print(u)
    #rmsd_var = c(rmsd_var$value,r);
    rmsd_var$value = c(rmsd_var$value,r);
    rmsd_var$name = c(rmsd_var$name,F2$name[u]);
    rmsd_var$start = NULL;
  }
